...
const APP = '240,239,238,233,217,211,210,209,203,202,197,196,195,194,193,178,151,146,145,141,140,139,138,137,134,131,130,128,116,115,114,113,112,111,110,96,92,83,82,81,79,76,74,72,55,54,225,224,223,222,221'
...
function MemberTypeSection() {
  ...
  const [joinCmpOn, setJoinCmpOn] = useState(false)
  const [joinRangeB, setJoinRangeB] = useState(null)
  const [busy, setBusy] = useState(false)
  const [prog, setProg] = useState({ msg: '', done: 0, total: 0 })
  const [error, setError] = useState(null)
  ...
  const joinFilterB = joinOn && joinCmpOn && joinRangeB?.start && joinRangeB?.end ? joinRangeB : null
  ...
  function rebuildSummary() {
    ...
    let a2 = null
    if (joinFilterB && aRef.current) {
      const { store } = aRef.current
      const snap = store.maxDate || iso(new Date())
      const cls2 = classifyMembers(store, snap, joinFilterB)
      a2 = { included: cls2.included, counts: cls2.counts,
             label: `${joinFilterB.start} ~ ${joinFilterB.end}` }
    }
    setSummary({ a, b: mk(bRef, false), a2 })
  }
  ...
  const clearFilter = () => { setJoinOn(false); setJoinRange(null); setJoinCmpOn(false); setJoinRangeB(null); setTimeout(rebuildSummary, 0) }
      ...
      <Grid columns={{ minWidth: countsKey === 'segment' ? 300 : 250, repeat: 'fit' }} gap={3}>
        {defs.map((d, i) => (
          <MemberTypeCard key={d.key} label={d.label}
                          ...
                          prev={summary.a2 ? summary.a2.counts[countsKey][i]
                                : summary.b ? summary.b.counts[countsKey][i] : null}
    ...
    <Stack gap={4}>
      ...
      {summary && (
        <SnapCard>
          <Stack direction="horizontal" gap={3} vAlign="end" wrap="wrap">
            {joinOn ? (
              <>
                <DateRangeInput label="회원가입 기간 (A)" value={joinRange} onChange={setJoinRange} presets={PRESETS} />
                {joinCmpOn ? (
                  <>
                    <DateRangeInput label="비교 가입기간 (B)" value={joinRangeB} onChange={setJoinRangeB} presets={PRESETS} />
                    <Button label="비교 제거" variant="ghost" size="sm"
                            clickAction={() => { setJoinCmpOn(false); setJoinRangeB(null); setTimeout(rebuildSummary, 0) }} />
                  </>
                ) : (
                  <Button label="+ 비교 기간" variant="secondary" size="sm" clickAction={() => {
                    if (!joinRangeB && joinRange?.start && joinRange?.end) {
                      const n = dayCount(joinRange.start, joinRange.end)
                      setJoinRangeB({ start: shiftDays(joinRange.start, -n), end: shiftDays(joinRange.end, -n) })
                    }
                    setJoinCmpOn(true)
                  }} />
                )}
            ...
            <Text type="supporting" color="secondary">
              {joinCmpOn
                ? '같은 파일 안에서 A 기간 가입자 vs B 기간 가입자를 비교해요 — 매일 데이터를 안 받아도 기간 비교 가능'
      ...
      {summary && (
        <>
          ...
          <Stack direction="horizontal" gap={2} vAlign="center" wrap="wrap">
            <Text type="label" weight="semibold">
              {joinFilter ? `기간 내 가입 ${comma(summary.a.included)}명${summary.a2 ? ` vs 비교기간 ${comma(summary.a2.included)}명` : ''} / 전체 ${comma(summary.a.n)}명` : `총 ${comma(summary.a.n)}명`}
            </Text>
            <Text type="supporting" color="secondary">
